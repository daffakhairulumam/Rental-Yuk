...
$pdf->Cell(0, 3, 'Tanggal Transaksi : ' . $row['tgl_pinjam'], 0, 1, 'L');
...
foreach ($data as $row) {
    $pdf->Cell(10, 3, $no, 0, 0, 'C');
    $pdf->Cell(13, 3, $row['merek'], 0, 0, 'C');
    $pdf->Cell(13, 3, $row['tgl_pinjam'], 0, 0, 'C');
    $pdf->Cell(13, 3, $row['tgl_kembali'], 0, 0, 'C');
    $pdf->Cell(13, 3, $row['harga'], 0, 0, 'C');
    $pdf->Cell(13, 3, $row['subtotal'], 0, 1, 'C');
    $no++;
...
$pdf->Cell(65, 3, 'Total: ' . number_format($totalTransaksi, 0, ',', '.'), 0, 1, 'R');

$pdf->SetXY($xPosition, $pdf->GetY());
$pdf->Cell(65, 3, 'Bayar: ' . number_format($bayar, 0, ',', '.'), 0, 1, 'R');

$pdf->SetXY($xPosition, $pdf->GetY());
$pdf->Cell(65, 3, 'Kembalian: ' . number_format($kembalian, 0, ',', '.'), 0, 1, 'R');

$pdf->SetFont('Arial', '', 5);
$pdf->Cell(0, 3, 'Dicetak Oleh : ' . $row['nama_petugas'], 0, 1, 'C');
